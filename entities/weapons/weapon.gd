...
func damage(body):
	...
	if is_network_master() && network.is_map_host():
		if body is Player && body.name != str(network.pid):
			network.peer_call_id(int(body.name), body, "damage", [DAMAGE, knockdir, TYPE])
		else:
			body.damage(DAMAGE, knockdir, TYPE)
		...
		if body is Player:
			...
			network.peer_call_id(network.get_map_host(), body, "damage", [DAMAGE, knockdir, TYPE])
	if delete_on_hit:
